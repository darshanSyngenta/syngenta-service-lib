...
export class LogFactory {
  ...
  configure(config:any) {
    if (config) {
      ...
      let transport = config.has('log.redis') && config.get('log.redis') && config.has('log.redis.host') && config.has('log.redis.port')?new redisTransport({host:config.get('log.redis.host'),port:config.get('log.redis.port')}) : new winston.transports.File({ filename: 'application.log' });
